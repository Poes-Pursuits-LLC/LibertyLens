...
import type { Analysis, CreateAnalysisInput, AnalysisFilter } from '~/core/analysis/analysis.model';
import { calculateLibertarianScore, analysisPrompts } from '~/core/analysis/analysis.model';
...
async function generateAnalysis(
  ...
  const mockPrinciples = [
    {
      ...
      explanation: 'The article discusses expanding government programs without considering market alternatives.'
    },
    {
      name: 'Free Markets',
      relevance: 'medium' as const,
      explanation: 'Economic interventions proposed would distort price signals and create inefficiencies.'
    }
  ];
  ...
  return {
    ...
    summary: 'This article advocates for increased government intervention without considering free-market alternatives or constitutional limitations.',
...
analysisRouter.get('/', requireAuth, zValidator('query', analysisFilterSchema), async (c) => {
  const userId = c.get('userId');
  const filters = c.req.valid('query');
  
  let analyses = Array.from(mockAnalysisDb.values())
  ...
  if (filters.minScore !== undefined) {
    analyses = analyses.filter(a => a.libertarianScore >= filters.minScore!);
  }
  if (filters.maxScore !== undefined) {
    analyses = analyses.filter(a => a.libertarianScore <= filters.maxScore!);
  }
  ...
  const nextCursor = startIndex + limit < analyses.length 
...
analysisRouter.post('/', requireAuth, zValidator('json', createAnalysisSchema), async (c) => {
  const userId = c.get('userId');
  ...
  const analysisResult = await generateAnalysis('Article content here', input.analysisSettings);
...
analysisRouter.get('/article/:articleId', requireAuth, async (c) => {
  ...
  const analysis = Array.from(mockAnalysisDb.values())
    .find(a => a.articleId === articleId && a.userId === userId);
...
analysisRouter.post('/:analysisId/regenerate', requireAuth, async (c) => {
  ...
  const newAnalysisResult = await generateAnalysis('Article content here', analysisSettings);
