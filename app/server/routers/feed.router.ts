...
import type { Feed, CreateFeedInput, UpdateFeedInput } from '~/core/feed/feed.model';
...
feedRouter.get('/', requireAuth, async (c) => {
...
// Create a new feed
feedRouter.post('/', requireAuth, zValidator('json', createFeedSchema), async (c) => {
  const userId = c.get('userId');
  const input = c.req.valid('json');
  
  // Check feed limit (e.g., 10 feeds for free tier)
  const userFeedCount = Array.from(mockFeedDb.values())
    .filter(feed => feed.userId === userId && feed.isActive).length;
  
  if (userFeedCount >= 10) {
    return c.json({ error: 'Feed limit reached. Upgrade to create more feeds.' }, 403);
  }
...
// Update a feed
feedRouter.patch('/:feedId', requireAuth, zValidator('json', updateFeedSchema), async (c) => {
  const userId = c.get('userId');
  const feedId = c.req.param('feedId');
...
feedRouter.post('/:feedId/refresh', requireAuth, async (c) => {
  ...
  if (feed.lastRefreshedAt) {
    ...
    if (minutesSinceRefresh < 5) { // 5 minute cooldown
      return c.json({ 
