...
export function SourcesPage(loaderData: Route.ComponentProps["loaderData"]) {
  const [selectedSourceIds, setSelectedSourceIds] = useState<string[]>([]);
  const [showAddCustomForm, setShowAddCustomForm] = useState(false);
  const [showAddToFeed, setShowAddToFeed] = useState(false);
  ...
  const error = (actionData as any)?.error || (loaderData as any)?.error;
  ...
  return (
    <div className="space-y-6">
      ...
      <NewsSourceList
        sources={(loaderData as any)?.sources || []}
      ...
      {selectedSourceIds.length > 0 && (
        <div className="mt-4 flex justify-end">
          <Button onClick={() => setShowAddToFeed(true)} variant="default">
            Add to Feed
          </Button>
        </div>
      )}
      ...
      {showAddToFeed && (
        <SourceAddToFeedModal
          ...
          onSuccess={() => {
            ...
            setShowAddToFeed(false)
          }}
          feeds={(loaderData as any)?.feeds || []}
          selectedSourceIds={selectedSourceIds}
          userId={(loaderData as any)?.userId}
        />
      )}
