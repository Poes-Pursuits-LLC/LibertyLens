      ...
      <div className="fixed inset-0 flex items-center justify-center p-4">
        <div className="w-full max-w-lg rounded-lg bg-white shadow-xl">
          <div className="p-4 border-b border-gray-200 flex items-center justify-between">
            <h2 className="text-lg font-semibold text-gray-900">Add to Feed</h2>
            <button className="text-gray-500 hover:text-gray-700" onClick={onClose} disabled={disabled}>
              Close
            </button>
          </div>

          {fetcher.data?.error && (
            <div className="mx-4 mt-4 rounded border border-red-300 bg-red-50 p-3 text-sm text-red-700">
              {fetcher.data.error}
            </div>
          )}
          ...
          <div className="p-4">
            {/* Mode toggle */}
            <div className="mb-4 inline-flex rounded-md border border-gray-200 p-1 bg-gray-50">
              <button
                type="button"
                className={`px-3 py-1 text-sm rounded ${mode === "existing" ? "bg-white shadow" : "text-gray-600"}`}
              ...
              <button
                type="button"
                className={`ml-1 px-3 py-1 text-sm rounded ${mode === "create" ? "bg-white shadow" : "text-gray-600"}`}
            ...
            {mode === "existing" ? (
              <div className="space-y-4">
                {feeds.length === 0 ? (
                  <div className="text-sm text-gray-600">You don’t have any feeds yet. Switch to “Create New”.</div>
                ) : (
                  <div className="space-y-2 max-h-64 overflow-auto pr-1">
                    {feeds.map((f) => (
                      <label key={f.id} className="flex items-center gap-2 p-2 rounded hover:bg-gray-50 cursor-pointer">
                        <input
                          type="radio"
                          name="feed"
                          value={f.id}
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label htmlFor="feedDescription" className="block text-sm font-medium text-gray-700 mb-1">Description (optional)</label>
                  <textarea
                    id="feedDescription"
                    value={description}
          ...
          <div className="flex items-center justify-between gap-2 border-t border-gray-200 p-4">
            ...
            <div className="flex gap-2">
              <button className="rounded px-4 py-2 text-gray-700 hover:bg-gray-100" onClick={onClose} disabled={disabled}>
                Cancel
              </button>
              {mode === "existing" ? (
                <button
