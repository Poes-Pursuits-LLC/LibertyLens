...
import type { Route } from "~/routes/dashboard/sources/routes/+types/index";
...
export const sourcesAction = async (args: Route.ActionArgs) => {
  try {
    ...
    switch (action) {
      ...
      case "addToFeed":
        return await addToFeed(formData);
      case "createFeedFromSources":
        return await createFeedFromSources(formData);
      default:
...
// New actions
async function addToFeed(formData: FormData) {
  try {
    const client = hc<AppType>(process.env.SERVER_URL!);
    const userId = String(formData.get("userId"));
    ...
    // Get existing feed
    const getRes = await client.feeds.getFeed.$get({ query: { feedId, userId } });
    if (!getRes.ok) {
      return { error: "Failed to fetch feed" };
    }
    const { feed } = await getRes.json();

    // Normalize existing source ids
    const existingIds: string[] = Array.isArray(feed?.sources)
      ? feed.sources.map((s: any) => (typeof s === "string" ? s : s.sourceId)).filter(Boolean)
      : [];

    const merged = Array.from(new Set([...existingIds, ...selected]));

    // Update feed
    const updRes = await client.feeds.updateFeed.$post({
      json: { feedId, userId, updateFields: { sources: merged } },
    });
    if (!updRes.ok) {
      const err = await updRes.json().catch(() => ({}));
      return { error: (err as any).error || "Failed to update feed" };
    }
...
async function createFeedFromSources(formData: FormData) {
  try {
    const client = hc<AppType>(process.env.SERVER_URL!);
    const userId = String(formData.get("userId"));
    ...
    const res = await client.feeds.createFeed.$post({
      json: { userId, name, description, sources: selected },
    });

    if (!res.ok) {
      const err = await res.json().catch(() => ({}));
      return { error: (err as any).error || "Failed to create feed" };
    }
